...
select a.name, count(f.FlightId) as no_of_flights
from airports a join flights f on a.airportid = f.origin
group by a.name order by count(f.FlightId)desc limit 1;
-- ----------------------------------------------------------------------------------------------------------------------

## Q2. Total number of ticekets sold per airline. #######################################################################

select a.name as Airline_name, count(t.ticketid) as tickets_sold
from airlines a join flights f on a.airlineid = f.airlineid join tickets t on f.flightid = t.flightid
group by a.name;
-- ----------------------------------------------------------------------------------------------------------------------

## Q3. List all the flights operated by 'Indigo' with airport names (Origin and Destination) ############################

select f.flightid, al.name as Airline_Name, a1.name as origin_airport_name, a2.name as destination_airport_name
from Flights f join airports a1 on f.origin = a1.airportid 
...
with cte as(
	...
	al.airlineid, al.name as airline_name, 
	count(f.flightid) as number_of_flights,
	rank() over(partition by a.name order by count(f.flightid) desc) as airline_rank
	from airports a left join flights f on a.airportid = f.origin
	join airlines al on f.airlineid = al.airlineid
	group by a.name,al.airlineid, al.name order by number_of_flights desc)
select airport_name, airline_name, number_of_flights from cte where airline_rank = 1;
-- ----------------------------------------------------------------------------------------------------------------------

## Q5. For each flight, show time taken in hours and categorize it in short(<2h), Medium(2-5h) or long(>5h) #############

-- **************** Assumption : Departure Time and Arrival Time are in same Timezone ***********************************
select FlightId, DepartureTime, ArrivalTime, TIMESTAMPDIFF(HOUR,DepartureTime,ArrivalTime) as Duration_Hours,
case 
	when TIMESTAMPDIFF(HOUR,DepartureTime,ArrivalTime) < 2 then 'Short'
    when TIMESTAMPDIFF(HOUR,DepartureTime,ArrivalTime) <5 then 'Medium'
    Else 'Long'
end as Flight_Category from flights;
-- ----------------------------------------------------------------------------------------------------------------------

## Q6. Show each passanger's first and last flight dates and number of flights. #########################################

select p.name,
MIN(f.DepartureTime) as First_Flight,
MAX(f.DepartureTime) as Last_Flight,
count(*) as number_of_flights
 from passengers p left join Tickets t on p.PassengerID = t.PassengerID
...
with cte as(
select f.flightid, f.Origin, f.Destination, t.ticketid, a1.name as origin_airport_name, a2.name as destination_airport_name, 
t.price, rank() over(partition by f.origin, f.destination order by t.price desc) as rnk
from flights f join tickets t on 
f.flightid = t.flightid
join airports a1 on f.origin = a1.airportid
join airports a2 on f.destination = a2.airportid)
select flightid, origin_airport_name, destination_airport_name, ticketid, price from cte where rnk = 1;
-- -----------------------------------------------------------------------------------------------------------------------

## Q8. Find the highest spending passenger in each frequent flyer status group. ##########################################

-- ----------------- To check the total spend by each customer in each frequent status group -----------------------------
select p.name,p.FrequentFlyerStatus, sum(t.price) as TotalSpend
from passengers p join tickets t
on p.passengerid = t.passengerid
group by p.name, p.frequentflyerstatus ;
-- ------------------------------------------------------------------------------------------------------------------------

with cte as
(select p.name,p.FrequentFlyerStatus, sum(t.price) as TotalSpend,
rank() over(partition by p.FrequentFlyerStatus order by sum(t.price) desc) as rnk
from passengers p join tickets t
on p.passengerid = t.passengerid
group by p.name, p.frequentflyerstatus)
select name,FrequentFlyerStatus, TotalSpend from cte where rnk = 1;
-- -----------------------------------------------------------------------------------------------------------------------

## Q9. Find the total revenue and number of ticekts sold by each airline and rank the airlines based in total revenue ####

select al.airlineid, al.name, count(t.Ticketid) as number_of_tickets, 
sum(t.price) as Total_Revenue,
rank() over(order by sum(t.price) desc) rnk
from airlines al left join flights f on al.airlineid = f.airlineid join Tickets t 
on f.flightid = t.flightid group by al.airlineid, al.name;
-- -----------------------------------------------------------------------------------------------------------------------
...
count(*) as Tickets_with_airline,
rank() over(partition by p.passengerid order by count(*) desc) as AirlineRank
from passengers p 
join tickets t on p.passengerid = t.passengerid 
join flights f on t.flightid = f.flightid 
join airlines a on f.airlineid = a.airlineid
group by p.passengerid, p.name, a.airlineid, a.name)
select passengerid, passengername, airlinename, Tickets_with_Airline from cte where AirlineRank = 1;
-- -----------------------------------------------------------------------------------------------------------------------
